...
function SectionHeader({
  ...
  return (
    <button
      onClick={onToggle}
      aria-expanded={open}
      className="flex w-full items-center justify-center gap-3 border-b border-steel-dark/50 pb-3"
    >
              ...
              {pastEvents.map((event) => (
                <li key={event.id}>
                  <Link
                    ...
                    className="steel-frame flex flex-col gap-1 bg-card p-4 transition-colors hover:bg-charcoal"
                  >
                    <span className="flex items-baseline justify-between gap-2">
                      <span className="font-display text-base font-semibold tracking-[0.15em] text-steel-bright uppercase">
                    ...
                    <span className="text-xs text-muted">
                      def. {event.runnerUp} ({event.finalScore}) · MVP{" "}
                      {event.mvp} · {event.teams} teams
                    </span>
                  </Link>
                </li>
              ))}
